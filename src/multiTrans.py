    ...
    def forward(
        ...
        **kwargs,
        ...
        pooled_output = torch.cat(
            [pooled_outputA, pooled_outputB, pooled_outputE], dim=1
        )
        logits = self.classifier(pooled_output)
        labelsCLS = labels
        lossCLS = None
        if labelsCLS is not None:
            ...
            if self.config.problem_type == "regression":
                ...
                if self.reweight == True:
                    ...
                    lossCLS = loss_fct(
                        logits.view(-1, self.num_labels), labelsCLS.view(-1)
                    )
                else:
                    loss_fct = CrossEntropyLoss()
                    lossCLS = loss_fct(
...
def compute_loss_rw(predictions, targets, w, criterion):
    ...
    if len(targets) > 0:
        ...
        losses = criterion(rearranged_output, rearranged_target).view(
